      ...
      <div className="flex flex-col justify-between h-full">
        <div>
          ...
          <div className="space-y-3">
            {genres.map((gen) => {
              return (
                <Genre onClick={() => handleGenresSelector(gen)} selected={selectedGenres.includes(gen)} key={gen}>
                  {gen}
                </Genre>
              );
            })}
...
const Genre = ({ children, selected, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={getSelectedStyle() + ' border-2 dark:border-dark-subtle border-light-subtle p-1 rounded mr-3'}
    >
      {children}
    </button>
  );
